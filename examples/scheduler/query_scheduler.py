...
def aggregate(tasks: List[Dict[str, Any]]) -> Dict[str, Dict[str, int]]:
    ...
    return {
        name: {status: counter.get(status, 0) for status in ALL_STATUSES}
...
def print_table(table: Dict[str, Dict[str, int]]) -> None:
    ...
    col_widths = {
        "TASK": max(len("TASK"), *(len(SUMMARY_LABEL if r == "__SUMMARY__" else r) for r in rows))
    }
    for status in ALL_STATUSES + ["TOTAL"]:
        col_widths[status] = max(
            len(status),
            *(len(str(
                sum(table[r].values()) if status == "TOTAL" else table[r][status]
            )) for r in rows)
        )
    ...
    def row(values):
        ...
        return "| " + " | ".join(
            f"{v:<{col_widths[h]}}" if h == "TASK" else f"{v:>{col_widths[h]}}"
    ...
    for name in rows[1:]:
        print(row(
            [name]
            + [str(table[name][s]) for s in ALL_STATUSES]
            + [str(sum(table[name].values()))]
        ))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Hostname or IP address of the Luigi scheduler."
    )
    parser.add_argument(
        "--scheduler-port",
        type=int,
        required=True,
        help="Port number of the Luigi scheduler."
    )
