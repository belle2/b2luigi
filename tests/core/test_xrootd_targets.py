...
class TestXrootDSystem(unittest.TestCase):
    def setUp(self):
        self.mock_server_path = "root://mockserver.cern.ch/"
        self.xrootd_system = XrootDSystem(self.mock_server_path)
        self.xrootd_system.client = MagicMock()
        self.xrootd_system.copy_file_to_remote = MagicMock()

        self.xrootd_target = XrootDTarget(self.mock_server_path, self.xrootd_system, "/scratch")
