...
def get_cli_arguments(ignore_additional_command_line_args=False):
    ...
    parser.add_argument(
        "--show-output", help="Instead of running the tasks, show which output files will/are created.", action="store_true"
    )
    parser.add_argument(
        "--test",
        help="Run the task list in test mode by printing the log directly to the screen instead" " of storing it in a file.",
        action="store_true",
    )
    parser.add_argument(
        "--batch", help="Instead of running locally, try to submit the tasks to the batch system.", action="store_true"
    )
    parser.add_argument("--batch-runner", help="Expert option to mark this worker as a batch runner.", action="store_true")
    parser.add_argument(
        "--dry-run", help="Do not run any task but set the return value to 0, if the tasks are complete.", action="store_true"
    )
    parser.add_argument(
