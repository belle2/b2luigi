...
def get_cli_arguments2(ignore_additional_command_line_args=False):
    """
    Get the command line arguments for testing purposes.
    """
    parser = argparse.ArgumentParser()

    commandparsers = parser.add_subparsers(dest="command", required=False)
    run_parser = commandparsers.add_parser("run", help="Run the tasks")
    ...
    remove_parser = commandparsers.add_parser("remove", help="Run the tasks")
    remove_parser.add_argument(
        "tasks", type=lambda s: s.split(",") if "," in s else [s], help="List of tasks to remove", default=[]
    )
    remove_parser.add_argument(
        "--only",
        type=lambda s: s.split(",") if "," in s else [s],
    ...
    remove_parser.add_argument(
        "-y", "--yes", action="store_true", help="Automatically confirm removal without prompting"
    )
    parser.add_argument(
        "--show-output",
        help="Instead of running the tasks, show which output files will/are created.",
